...
pub fn run_postprocess_for_path_inner(
    ...
    // 兜底补删：视频文件仍存在，但原始 pipeline 中某启用节点开启了"上传后自动删除"
    // 且其合并结果为成功（上次上传成功但当时未删除，本次重跑该模块被跳过，不会重复上传）。
    // Fallback deletion: the video file still exists, but an enabled node in the original
    // pipeline has "delete after upload" enabled and its merged result is success
    // (it uploaded successfully before but the file was never deleted; the module is
    // skipped on this re-run, so the file is deleted here without re-uploading).
    if video_path.exists() {
        let pending_delete = pipeline
            .nodes
            .iter()
            .filter(|n| n.enabled)
            .any(|n| {
                let wants_delete = n
                    .params
                    .get("delete_after_upload")
                    .and_then(|v| v.as_bool())
                    .unwrap_or(false);
                wants_delete
                    && results
                        .iter()
                        .any(|r| r.module_id == n.module_id && r.success)
            });
        if pending_delete {
            // 统一走 remove_recording_files：视频 + sidecar 封面图 + meta 一并删除
            // Use remove_recording_files so the video, sidecar covers, and meta are all deleted
            match crate::recording::meta::remove_recording_files(video_path) {
                Ok(()) => {
                    tracing::info!(
                        "上传已成功，按设置补删本地文件（含封面与 meta）/ Upload already succeeded, deleting local file (incl. cover & meta) per setting: {:?}",
                        video_path
                    );
                }
                Err(e) => {
                    tracing::error!(
                        "补删本地文件失败 / Fallback deletion failed {:?}: {}",
                        video_path, e
                    );
                }
            }
