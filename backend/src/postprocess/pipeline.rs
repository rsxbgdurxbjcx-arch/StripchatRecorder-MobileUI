...
/// 清空 `exe_dir/tmp`：删除所有直接子文件和所有 `_split_tmp_*` 目录（不按时间过滤）。
/// 供每 1 分钟运行一次的定时任务调用。Linux 下删除仍被进程占用的文件不会中断
/// 进行中的上传——已打开的文件句柄保持有效，磁盘空间在进程退出后释放。
///
/// Clear `exe_dir/tmp`: remove ALL direct child files and ALL `_split_tmp_*`
/// directories (no age filter). Called by the periodic task that runs every minute.
/// On Linux, unlinking a file still held open by a process does not interrupt an
/// in-progress upload — the open file handle stays valid and the disk space is
/// freed when the process exits.
pub fn clear_tmp_dir() {
    let tmp = exe_dir().join("tmp");
    if !tmp.is_dir() {
        return;
    }
    ...
    if removed_files + removed_dirs > 0 {
        tracing::info!(
            "tmp clear: removed {} file(s) and {} _split_tmp_* dir(s) / 清空临时目录：{} 个文件、{} 个分割临时目录",
            removed_files, removed_dirs, removed_files, removed_dirs
        );
    }
}
...
pub fn run_pipeline(
    ...
    for node in all_nodes.iter() {
        ...
        // 若模块请求删除其输入文件，由主程序执行删除（同时清理 sidecar 封面图和 meta 文件）。
        // 删除失败时重试 3 次（每次间隔 500ms，在 remove_recording_files 内部），仍失败则记录 error。
        // If the module requested deletion of its input file, the host performs the deletion
        // (also cleaning up sidecar cover images and the meta file). Retries up to 3 times
        // (500ms apart, inside remove_recording_files), then logs an error on failure.
        if result.delete_input {
            // 先删除本次运行中前置节点产生的中间输出文件（如 contact_sheet 预览图），
            // 避免输入文件删除后它们成为孤儿（meta 中的 module_outputs 记录要等流水线
            // 结束后才写入，届时删除已来不及）
            // Delete intermediate output files produced by earlier nodes in THIS run
            // (e.g. contact_sheet previews) first, so they don't become orphans once the
            // input is gone (meta module_outputs are only written after the pipeline ends)
            for prev in &results {
                    ...
                    && out.exists()
                {
                    if let Err(e) = std::fs::remove_file(out) {
                        tracing::warn!("DELETE_INPUT: failed to remove intermediate output {:?}: {}", out, e);
                    } else {
                        tracing::info!("DELETE_INPUT: removed intermediate output {:?}", out);
                    }
                }
            }
            match crate::recording::meta::remove_recording_files(&current_input) {
                Ok(()) => {
                    tracing::info!(
                        "DELETE_INPUT: removed {:?} (video + cover + meta)",
                        current_input
                    );
                }
                Err(e) => {
                    tracing::error!(
                        "DELETE_INPUT: failed to remove {:?} after 3 attempts: {}",
                        current_input, e
                    );
                }
            }
...
fn run_node(
    ...
    NodeResult {
        ...
        // 失败时也尊重模块的显式删除请求（如"上传失败自动删除"场景）
        // Honor the module's explicit deletion request even on failure
        // (e.g. the "delete local file on upload failure" scenario)
        delete_input,
    }
}
