...
/// ffmpeg 缺失警告是否已推送过（只推一次，避免 SSE 重连时重复弹窗）
/// Whether the ffmpeg-missing warning has been pushed (push only once to avoid
/// duplicate toasts on SSE reconnect)
static FFMPEG_MISSING_PUSHED: AtomicBool = AtomicBool::new(false);
...
async fn sse_handler(
    ...
    // 首个客户端连接时推送一次 ffmpeg 缺失警告（与桌面端行为一致；
    // 启动时直接广播会被尚未连接的客户端错过，因此在 subscribe 之后推入广播通道）
    // Push the ffmpeg-missing warning once when the first client connects (matching
    // desktop behavior; a startup-time broadcast would be missed by clients that
    // connect later, so it is pushed into the broadcast channel after subscribe)
    if !FFMPEG_MISSING_PUSHED.swap(true, Ordering::Relaxed)
        && !crate::recording::recorder::ffmpeg_available()
    {
        s.emitter.emit(
            "ffmpeg-missing",
            &serde_json::json!({
...
pub async fn run_server(port: u16) {
    ...
    // 每 1 分钟触发一次 tmp 清空任务，但仅在没有后处理任务运行时执行：
    // 后处理流水线全程持有 pp_lock，try_lock 失败说明有模块正在工作（上传中/分割中），
    // 本轮直接跳过，绝不误删进行中的临时文件；空闲时全量清空（含 _split_tmp_* 目录），
    // 孤儿文件最多存活约 1 分钟，磁盘不会被临时文件悄悄占满。
    // Fire a tmp cleanup every 1 minute, but only when no post-processing task is
    // running: the pipeline holds pp_lock for its entire run, so a failed try_lock
    // means a module is actively working (uploading/splitting) and this round is
    // skipped — in-flight temp files are never touched. When idle, the tmp dir is
    // fully cleared (including _split_tmp_* dirs), so orphaned files survive at most
    // ~1 minute and the disk can never be silently filled by temp files.
    {
        let state_for_tmp = Arc::clone(&app_state);
        tokio::spawn(async move {
            let mut ticker = tokio::time::interval(std::time::Duration::from_secs(60));
            ticker.tick().await; // 首次 tick 立即返回，跳过启动时刻 / first tick fires immediately; skip it
            loop {
                ticker.tick().await;
                let state = Arc::clone(&state_for_tmp);
                let _ = tokio::task::spawn_blocking(move || {
                    // 尝试获取后处理串行锁：拿不到说明有流水线正在运行，本轮跳过；
                    // 拿到了则在持锁期间清空，新的流水线也无法插队，无竞态。
                    // Try to acquire the post-processing serial lock: if unavailable,
                    // a pipeline is running — skip this round. If acquired, clear
                    // while holding it so no new pipeline can race in.
                    let Ok(_guard) = state.pp_lock.try_lock() else {
                        tracing::debug!("tmp clear skipped: post-processing is running / 跳过清空：后处理运行中");
                        return;
                    };
                    crate::postprocess::pipeline::clear_tmp_dir();
                })
                .await;
            }
        });
