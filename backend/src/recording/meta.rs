...
/// 删除一个录制对应的全部文件：视频文件、同目录下的 sidecar 封面图（webp/jpg/jpeg/png）、
/// meta 文件中记录的模块输出文件（如 contact_sheet 预览图），以及 meta 文件本身。
/// 视频删除失败时最多重试 3 次（每次间隔 500ms）；文件已不存在视为成功。
///
/// Delete all files belonging to a recording: the video file, sidecar cover images
/// (webp/jpg/jpeg/png) in the same directory, module output files recorded in the meta
/// (e.g. contact_sheet preview images), and the meta file itself.
/// Retries video deletion up to 3 times (500ms apart) on failure;
/// an already-missing video file is treated as success.
pub fn remove_recording_files(video_path: &Path) -> std::io::Result<()> {
    // 先读出 meta 中记录的模块输出文件路径（meta 稍后会被删除，之后就拿不到了）
    // Read module output paths from meta first (meta is deleted below, after which
    // this information is no longer available)
    let module_outputs: Vec<PathBuf> = read_meta(video_path)
        .and_then(|m| m.module_outputs)
        .map(|outputs| outputs.into_values().map(PathBuf::from).collect())
        .unwrap_or_default();
    ...
    for attempt in 1..=3 {
        match std::fs::remove_file(video_path) {
            ...
            Err(e) => {
                tracing::warn!(
                    "remove_recording_files: remove {:?} failed (attempt {}/3): {}",
                    video_path,
                    attempt,
                    e
                );
    ...
    // 删除同目录下的 sidecar 封面图 / Delete sidecar cover images in the same directory
    if let Some(parent) = video_path.parent()
        && let Some(stem) = video_path.file_stem().and_then(|s| s.to_str())
    {
    ...
    // 删除模块输出文件（contact_sheet 预览图等），避免 meta 删除后它们成为孤儿文件
    // Delete module output files (e.g. contact_sheet previews) so they don't become
    // orphaned once the meta file is gone
    for out in &module_outputs {
        // 不删除视频自身（理论上不会包含，防御性检查）/ Never delete the video itself (defensive)
        if out != video_path && out.exists() {
            if let Err(e) = std::fs::remove_file(out) {
                tracing::warn!("remove_recording_files: failed to remove module output {:?}: {}", out, e);
            } else {
                tracing::info!("remove_recording_files: removed module output {:?}", out);
            }
        }
    }

    delete_meta(video_path);
    Ok(())
}
