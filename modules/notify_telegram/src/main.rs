...
/// 清理本次运行在 tmp 目录中产生的所有临时文件（成功与失败路径都会调用）。
/// 仅删除以本文件 stem 开头的临时文件，不会误删其他并发任务的 tmp 文件。
///
/// Clean up all temporary files produced by this run in the tmp directory
/// (called on BOTH success and failure paths). Only deletes temp files whose
/// names start with this file's stem, so concurrent tasks are never affected.
///
/// # 参数 / Parameters
/// - `stem`: 输入文件的 file_stem / File stem of the input file
/// - `input`: 原始输入视频路径（永不删除）/ Original input video path (never deleted)
/// - `video_parts`: split_video 返回的片段列表 / Segment list returned by split_video
fn cleanup_tmp_leftovers(stem: &str, input: &Path, video_parts: &[PathBuf]) {
    // 1. 分割产生的临时片段（部分可能已在上传内部清理，重复删除无害）
    // Temporary segments from splitting (some may already be removed; re-removal is harmless)
    for part in video_parts {
        if part != input {
    ...
    // 2. 按文件名模式扫描 tmp 目录，清除本次运行的所有残留：
    //    {stem}_part*           分割片段及其转换文件（{stem}_part000_tg_tmp.mkv）和缩略图（{stem}_part000.tg_thumb.png）
    //    {stem}_tg_tmp.*        封面/视频格式转换临时文件
    //    {stem}.tg_thumb.png    视频缩略图
    //    {stem}_tg_resized.jpg  缩放后的封面图
    // Scan the tmp directory by filename pattern to remove all leftovers of this run:
    //    {stem}_part*           split segments + their converted files/thumbnails
    //    {stem}_tg_tmp.*        cover/video format-conversion temp files
    //    {stem}.tg_thumb.png    video thumbnail
    //    {stem}_tg_resized.jpg  resized cover image
    let part_prefix = format!("{}_part", stem);
    let tg_tmp_prefix = format!("{}_tg_tmp.", stem);
    let thumb_name = format!("{}.tg_thumb.png", stem);
    let resized_prefix = format!("{}_tg_resized", stem);
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            // 仅删除空的 _split_tmp_* 残留目录；非空目录可能属于正在运行的并发任务
            // Only remove empty _split_tmp_* leftover dirs; non-empty dirs may
            // belong to a concurrently running task
            if name.starts_with("_split_tmp_") {
                let _ = fs::remove_dir(&path);
            }
            continue;
        }
        if name.starts_with(&part_prefix)
            || name.starts_with(&tg_tmp_prefix)
            || name == thumb_name
            || name.starts_with(&resized_prefix)
        {
...
/// 清理 tmp 目录中的"孤儿"临时文件：修改时间早于 `max_age` 的文件一律删除。
/// 这些文件只可能是被杀死/取消/崩溃的模块进程遗留的（正常运行的模块在退出前
/// 会自行清理），其中分割片段是整份视频副本，长期累积可占满磁盘。
///
/// Prune orphaned temp files in the tmp directory: anything older than `max_age`
/// is deleted. Such files can only be leftovers of killed/cancelled/crashed module
/// processes (a healthy module cleans up before exit); split segments are full-size
/// video copies that can fill the disk over time.
fn prune_stale_tmp(max_age: Duration) {
    ...
    for entry in entries.flatten() {
        ...
        if path.is_dir() {
            // 仅清理分割临时目录（_split_tmp_*），其他目录不碰
            // Only prune split temp dirs (_split_tmp_*); leave other directories alone
            if entry.file_name().to_str().map(|n| n.starts_with("_split_tmp_")).unwrap_or(false) {
...
fn run() -> Result<(), String> {
    ...
    let delete_on_failure = param_bool("delete_on_failure", false);
    ...
    let video_parts: Vec<PathBuf> = if send_video {
        match split_video(&input, TG_MAX_BYTES) {
            Ok(parts) => parts,
            Err(e) => {
                // 切分失败也要清理可能已产生的临时片段，避免残留占用磁盘
                // Clean up any segments produced before the split failure to avoid disk bloat
                cleanup_tmp_leftovers(stem, &input, &[]);
                return Err(e);
            }
        }
    } else {
    ...
    // 构建 Tokio 运行时并执行异步上传，最多重试 3 次（捕获结果而非直接传播错误）
    // Build Tokio runtime and execute async upload with up to 3 retries
    // (capture the result instead of propagating the error immediately)
    let upload_result: Result<(), String> = match tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()
    {
    ...
    // 无论上传成功或失败，统一清理本次运行产生的所有临时文件
    // （分割片段、格式转换临时文件、缩略图、缩放封面、空的分割临时目录），
    // 避免上传失败时整份视频副本残留在 tmp 目录中。
    // Regardless of upload success or failure, clean up ALL temp files produced
    // by this run (split segments, format-conversion temp files, thumbnails,
    // resized covers, empty split temp dirs), so a failed upload never leaves a
    // full copy of the video behind in the tmp directory.
    cleanup_tmp_leftovers(stem, &input, &video_parts);

    // 上传失败：若开启了"上传失败自动删除"，则请求主程序删除本地文件，仍以失败码退出
    // Upload failed: if "delete on failure" is enabled, ask the host to delete the
    // local file, but still exit with a failure code so the UI shows the failure
    if let Err(ref e) = upload_result {
        if delete_on_failure {
            println!("DELETE_INPUT");
            // 显式 flush，确保主程序在进程退出前读到协议行
            // Flush explicitly so the host reads the protocol line before exit
            use std::io::Write;
            let _ = std::io::stdout().flush();
            eprintln!(
                "上传失败，按设置删除本地文件 / Upload failed, deleting local file per setting: {}",
                input.display()
            );
            // 触发"上传失败自动删除"时，同步清理 6 小时前的孤儿临时文件（含 _split_tmp_* 目录）
            // When "delete on failure" fires, also prune orphaned temp files older than
            // 6 hours (including _split_tmp_* dirs)
            prune_stale_tmp(Duration::from_secs(6 * 3600));
        }
        return Err(e.clone());
    }
    ...
    if delete_after_upload {
        ...
        // 触发"上传后自动删除"时，同步清理 6 小时前的孤儿临时文件（含 _split_tmp_* 目录）
        // When "delete after upload" fires, also prune orphaned temp files older than
        // 6 hours (including _split_tmp_* dirs)
        prune_stale_tmp(Duration::from_secs(6 * 3600));
    } else {
...
fn main() {
    ...
    // 确保临时目录存在，并清理 6 小时前的孤儿临时文件（被杀进程遗留的视频副本）
    // Ensure temp directory exists, and prune orphaned temp files older than 6 hours
    // (video copies left behind by killed processes)
    tmp_dir();
    prune_stale_tmp(Duration::from_secs(6 * 3600));
    if let Err(e) = run() {
        eprintln!("{}", e);
